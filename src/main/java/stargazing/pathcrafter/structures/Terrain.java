...
public class Terrain {
    ...
    public Terrain(int startX, int startZ, int endX, int endZ) {
        // Establish ranges
    ...
    public void genColumns() {
        int chunkMinX = minX / CHUNK_SIZE;
        int chunkMinZ = minZ / CHUNK_SIZE;
        int chunkMaxX = maxX / CHUNK_SIZE + 1;
        int chunkMaxZ = maxZ / CHUNK_SIZE + 1;
    ...
    public ArrayList<Vertex> findVerticesAt(int x, int z) {
        // Look for vertices at integer block coordinates
        // i.e. the corners of blocks
        // For now, this simple algo will do.
        ...
        while (i0 < column0.surfaces.size() ||
            ...
            switch (matchState) {
                ...
                case 1:
                    if (state1 && state2) {
                        vertices.add(createVertex(x, curY, z, 0));
                    }
                    else if (state1) {
                        vertices.add(createVertex(x, curY, z, 2));
                    }
                    else if (state2) {
                        vertices.add(createVertex(x, curY, z, 1));
                    }
                    else if (state3) {
                        vertices.add(createVertex(x, curY, z, 0));
                        vertices.add(createVertex(x, curY, z, 1));
                        ...
                        vertices.add(createVertex(x, curY, z, 3));
                    }
                    break;
                case 2:
                    if (state0 && state3) {
